...
from app.api.common import filter_items, SessionDep, has_default_filters, filter_movies, FilterParams
...
@router.get("/")
async def get_genres(session: SessionDep, filter_query: Annotated[FilterParamsBasic, Query()]) -> list[Item]:
  query = select(Genre)
...
async def get_genre(session: SessionDep, genre_id: Annotated[int, Path(ge=0)]) -> Item: 
  ...
  if not genre:
      raise HTTPException(status_code=404, detail=f"No Genre found with id={genre_id}.")
  return genre
