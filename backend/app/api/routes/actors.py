...
@router.get("/")
async def get_actors(session: SessionDep, filter_query: Annotated[FilterParamsBasic, Query()]) -> list[Item]:
  query = select(Actor)
...
async def get_actor(session: SessionDep, actor_id: Annotated[int, Path(ge=0)]) -> Item: 
  ...
  if not actor:
      raise HTTPException(status_code=404, detail=f"No Actor found with id={actor_id}.")
  return actor
